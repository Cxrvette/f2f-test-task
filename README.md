...
Локаторы и пользовательские действия централизованы в `tests/pages/` по
паттерну Page Object Model. Для стабильного контракта между UI и тестами
используются атрибуты `data-testid`. Тесты запускаются последовательно, поскольку
демонстрационный backend нестабилен при конкурентных операциях авторизации и БД.
